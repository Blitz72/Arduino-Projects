...
//
//  public:
//    void setAddress(byte add0, byte add1, byte add2, byte add3) {
//      address[0] = add0;
//      address[1] = add1;
//      address[2] = add2;
//      address[3] = add3;
//    }
//    void setMins(char m10, char m1) {
//      mins[0] = m10;
//      mins[1] = m1;
//    };
//    void setSecs(char s10, char s1) {
//      secs[0] = s10;
//      secs[1] = s1;
//    };
//    int getTimer() {
//      return ((mis[0]-0x30) * 10 + (mins[1]-0x30)) + ((secs[0]-0x30) * 10 + (secs[1]-0x30));
//    };
//    void printInfo() {
//      
//    };
...
//  Serial.print(F("An: "));         Serial.println(cal.An);
//  Serial.print(F("Bn: "));         Serial.println(cal.Bn);
//  Serial.print(F("Cn: "));         Serial.println(cal.Cn);
//  Serial.print(F("Dn: "));         Serial.println(cal.Dn);
//  Serial.print(F("En: "));         Serial.println(cal.En);
//  Serial.print(F("Fn: "));         Serial.println(cal.Fn);
//  Serial.print(F("Divider: "));    Serial.println(cal.divider);
//  Serial.print(F("Calibrated: ")); Serial.println(cal.calibrated);
...
//  tft.drawRect(0, 0, tft.width() - 1, 40, RA8875_WHITE);
  ...
  tft.fillRect(1, 1, tft.width() - 3, 37, gray);
...
//  Serial.print(F("drawArray elapsed time: "));
...
//  endTime = millis();
//  Serial.print(F("drawArray2 elapsed time: "));
...
void loop() {
  ...
  if(tft.touched()) {
    ...
    calibrateTSPoint(&calibrated, &raw, &cal );
//    Serial.print(F("calibrated.x: "));   Serial.println(calibrated.x);
//    Serial.print(F("calibrated.y: "));   Serial.println(calibrated.y);
...
void updateInfo(byte address[], char minutes[], char seconds[], char _status, char latitude[], char longitude[], char battStatus) {
  ...
  if (_latitude >= 54020 && _latitude <= 54057 && _longitude >= 56060 && _longitude <= 56141) {
    tft.textWrite("SECURITY");
  } else if (_latitude >= 54201 && _latitude <= 54272 && _longitude >= 56226 && _longitude <= 56324) {
    tft.textWrite("B307");
  } else if (_latitude >= 53951 && _latitude <= 53975 && _longitude >= 55891 && _longitude <= 55930) {
    tft.textWrite("B330 APPS LAB");
  } else {
...
//  ((address[3] & 0x0f) < 10) ? add[7] = (address[3] & 0x0f) + 0x30 : add[7] = (address[3] & 0x0f) + 0x37;
  ...
  switch (battStatus) {
    case 'H':
      drawArray2(435, 51, 18, 36, highBattIcon);
      break;
    case 'M':
      drawArray2(435, 51, 18, 36, medBattIcon);
      break;
    case 'L':
      drawArray2(435, 51, 18, 36, lowBattIcon);
      break;
  }
...
void drawArray2(int x, int y, int height, int width, uint16_t * file) {
  ...
  for (int row = y; row < y + height; row++) {
    ...
    tft.drawPixels(buff, width, x, row);
  }
...
int calibrateTSPoint( tsPoint_t * displayPtr, tsPoint_t * screenPtr, CalibrationObject * _cal ) {
  int  retValue = 0 ;
  
  if( _cal->divider != 0 ) {
    displayPtr->x = ( (_cal->An * screenPtr->x) + 
                      (_cal->Bn * screenPtr->y) + 
                       _cal->Cn 
                    ) / _cal->divider ;

    displayPtr->y = ( (_cal->Dn * screenPtr->x) + 
                      (_cal->En * screenPtr->y) + 
                       _cal->Fn 
                    ) / _cal->divider ;
  }
...
//  Serial.print(F("displayPtr: "));   Serial.print(displayPtr->x);
//  Serial.print(F(", "));             Serial.println(displayPtr->y);
...
void waitForTouchEvent(tsPoint_t * point) {
  ...
  if (tft.touched()) {
    ...
    Serial.print(point->x); Serial.print(F(", ")); Serial.println(point->y);
  }
  else {
    point->x = 0;
    point->y = 0;
