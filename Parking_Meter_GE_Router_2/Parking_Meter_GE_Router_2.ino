...
#define LOW_BATTERY_LIMIT 528           // R1 = 106.5kOkms, R2 = 39.6kOhms; 12.56 volts = Analog In of 698, 528 = 9.5V

//#define GPS_TESTING_INTERVAL 5000
#define GPS_INTERVAL 60000  // time in mS: 1 minute
...
//  -------------------------------------------------------------- xbeeTransmit ------------------------------------------------------------------------
void xbeeTransmit (byte options, byte mins, byte secs, char _status, char msg[]) {
...
//  xbeeSerial.write(coordinator, 8);
  ...
  xbeeSerial.write(msg, sizeof(msg));
...
void checkBatt() {
  ...
  float batt = battAnalogIn * 0.0194;
  Serial.print("Battery Voltage = ");
  Serial.println(batt, 2);

  String msg = "LOW BATTERY";
  msg.toCharArray(msgArray, 13);
  
  if (battAnalogIn < LOW_BATTERY_LIMIT) {
    xbeeTransmit(TX_ACK, 0x00, 0x00, 'X', msgArray);
  }
