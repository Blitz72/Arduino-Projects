...
#define lightCount 64  //Total # of lights on string (usually 50, 48, or 36 or 64 for a custom string!)
...
void setup() {
  ...
  DDRB = DDRB | portB;  //current value = 0x03 so only first two pins on PORTB will be enabled
...
void loop(){
  for (brightness = 0; brightness < 0xCC; brightness++){
    for (byte i = 0; i < 12; i++){
      setColor(0x01, i, brightness, white);
      setColor(0x02, i, brightness, blue);
    }
    for (int i = 12; i < 46; i++){
      setColor(0x01, i, brightness, red);
      setColor(0x02, i, brightness, blue);
    }
    for (int i = 46; i < lightCount; i++){
      setColor(0x01, i, brightness, red);
      setColor(0x02, i, brightness, white);
    }
  }
  
  delay(delayTime * 2);
  
  for (brightness = 0xCC; brightness >= 0; brightness--){
    for (byte i = 0; i < 12; i++){
      setColor(0x01, i, brightness, white);
      setColor(0x02, i, brightness, blue);
    }
  ...
  for (brightness = 0; brightness < 0xCC; brightness++){
    for (int i = 0; i < lightCount; i++){
      setColor(0x03, i, brightness, green);
    }
  }
  ...
  for (brightness = 0; brightness < 0xCC; brightness++){
    for (byte i = 0; i < 12; i++){
      setColor(0x01, i, brightness, yellow);
      setColor(0x02, i, brightness, yellow);
    }
    for (int i = 12; i < 46; i++){
      setColor(0x01, i, brightness, red);
      setColor(0x02, i, brightness, yellow);
  ...
  for (brightness = 0; brightness < 0xCC; brightness++){
    for (int i = 0; i < lightCount; i++){
      setColor(0x03, i, brightness, blue);
    }
  }
